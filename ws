...
Library.MinSize = if Library.IsMobile then Vector2.new(550, 200) else Vector2.new(750, 350);
...
function Library:SetDPIScale(value: number) 
    ...
    DPIScale = value / 100;
    Library.MinSize = (if Library.IsMobile then Vector2.new(550, 200) else Vector2.new(750, 350)) * DPIScale;
end;
...
function BaseGroupboxFuncs:AddSlider(Idx, Info)
    ...
    -- ULTIMATE FIXED SLIDER - Uses MouseButton1Down/MouseButton1Up and Touch events separately
    ...
    local function StartDrag(input)
        ...
        isDragging = true;
        dragStartX = Fill.AbsoluteSize.X;
        dragStartMouseX = Mouse.X;
        startValue = Slider.Value;
        
        -- Disable scrolling on sides
        local Sides = {};
    ...
    local function UpdateDrag()
        ...
        local deltaX = Mouse.X - dragStartMouseX;
        local newWidth = math.clamp(dragStartX + deltaX, 0, Slider.MaxSize);
        local progress = Library:MapValue(newWidth, 0, Slider.MaxSize, 0, 1);
        local newValue = Slider:GetValueFromXScale(progress);
    ...
    -- Touch events
    SliderInner.TouchTap:Connect(function(input)
        StartDrag(input)
    end)

    SliderInner.TouchLongPress:Connect(function(input)
        StartDrag(input)
    end)

    -- Update on mouse movement
    Library:GiveSignal(Mouse.Move:Connect(function()
        UpdateDrag()
    end))
    ...
    -- Also end drag if mouse leaves the slider
    SliderInner.MouseLeave:Connect(function()
        if isDragging and InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) == false then
            EndDrag()
        end
    end)
...
-- ============================================================
--  WINDOW CREATION - HORIZONTAL TABS
-- ============================================================
function Library:CreateWindow(...)
    ...
    if typeof(Config.Size) ~= 'UDim2' then
        if Library.IsMobile then
            ...
            Config.Size = UDim2.fromOffset(800, math.clamp(ViewportSizeYOffset, 200, 600))
        else
            Config.Size = UDim2.fromOffset(850, 420)
        end
    end
    ...
    Library:MakeDraggable(Outer, 30, true);
    ...
    function Window:AddTab(Name)
        ...
        function Tab:ShowTab()
            ...
            local textBounds = Library:GetTextBounds(Name, Library.Font, 14)
            TabButton.Size = UDim2.new(0, textBounds + 20, 0, 24)
        end;
